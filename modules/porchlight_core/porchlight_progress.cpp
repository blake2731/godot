...
bool PorchlightProgress::complete_milestone(
    ...
    const Error save_error = save_progress();
...
Error PorchlightProgress::save_progress() {
    ...
    for (int index = 0;
        ...
        saved_milestones[index] =
                String(completed_milestones[index]);
    }
...
Error PorchlightProgress::load_progress() {
    ...
    if (saved_value.get_type() != Variant::ARRAY) {
        return ERR_INVALID_DATA;
    }

    const Array saved_milestones = saved_value;
    ...
    for (int index = 0;
            ...
            index++) {
        const Variant entry = saved_milestones[index];

        if (entry.get_type() != Variant::STRING &&
                entry.get_type() != Variant::STRING_NAME) {
            continue;
        }
...
String PorchlightProgress::get_save_path() const {
    return SAVE_PATH;
}
